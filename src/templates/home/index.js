...
const requireContext = require.context('../../assets/logo/home/', false, /\.(png|jpe?g|svg|gif)$/);

const LOGOS = {};
requireContext.keys().forEach(key => {
    const filename = key.replace(/^\.\//, '').replace(/\.\w+$/, '');
    LOGOS[filename] = requireContext(key);
});
export default {
    template: TEMPLATE,
    navbar: {
        companyName: companyName,
        logo: LOGOS[`h${companyLogo}`],
        links: [
    ...
    about: {
        ...
        content: `${companyName} is a real estate marketing brand dedicated to helping homeowners stay informed. We believe that education and transparency are the keys to success in real estate — whether you're selling now or just watching the market. In addition to buying properties, we work with some of the top agents in all areas of the United States. So wether you are looking to get a quick Timeline to Close on your property, or looking to put your property on the market, we are able to help with whatever your needs consist of.`,
    ...
    compare: {
        ...
        backgroundImageUrl: IMAGES[IMAGES.length - companyImage],
        tableHeaders: {
            category: "Category",
